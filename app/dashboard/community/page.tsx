        ...
        {activeTab === 'overview' && (
          <div className="space-y-4">
            <div className="bg-gradient-to-br from-forest-50 to-emerald-50 rounded-lg p-6 border border-forest-300">
              <h2 className="text-2xl font-bold text-navy-900 mb-4">{mockBreedCommunity.breed}</h2>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs text-gray-700">Community Members</p>
                  <p className="text-3xl font-bold text-navy-900">{mockBreedCommunity.memberCount}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-700">Total Dogs</p>
                  <p className="text-3xl font-bold text-navy-900">{mockBreedCommunity.totalDogs}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-xs text-gray-700">Average Weight</p>
                  <p className="text-2xl font-bold text-navy-900">{mockBreedCommunity.avgWeight} lbs</p>
                </div>
              </div>
            </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-700">{member.breed}</p>
                      {member.taskType && (
                        <p className="text-xs text-gray-700 mt-1 capitalize">{member.taskType.replace('-', ' ')} Service Dog</p>
                      )}
                      {member.location && (
                        <p className="text-xs text-gray-700 mt-1">📍 {member.location}</p>
                      )}
                    </div>
                  </div>
                </CardContent>
